...
export default {
  async asyncData({ route, $axios }) {
    ...
    let product = await $axios.get(
       `http://80.249.151.147/api/v1/present/${product_id}`
    );
    const comments = await $axios.get(
      `http://80.249.151.147/api/v1/present/comments/all/${product_id}`
    );
    ...
    addBasket(present_id){
     ...
     if(this.$store.state.localStorage.uid_auth_user){
      ...
      let data = {
        ...
        this.$axios
        .$post(`http://80.249.151.147/api/v1/present/users/basket/user_basket/add`,data, {
          headers: headers,
        })
        .then((resp) => {
          console.log(resp);
      ...
      this.$axios
        .$post(`http://80.249.151.147/api/v1/present/users/basket/anonim_basket/add`,data, {
          headers: headers,
        })
        .then((resp) => {
