...
export default {
  ...
  methods: {
    retbonus() {
        this.$axios
        .$get(`http://api-booking.ru/api/v1/present/users/${this.$store.state.localStorage.uid_auth_user}`,{
        })
        .then((resp) => {
    ...
    createUser_googleauth(uid_user,email){
      ...
      this.$axios
        .$post(`http://api-booking.ru/api/v1/present/users/anonim/${this.$store.state.localStorage.basket.id_basket}`, data, {
          headers: headers,
        })
        .then((resp) => {
      ...
      this.$axios
        .$post(`http://api-booking.ru/api/v1/present/users/`, data, {
          headers: headers,
        })
        .then((resp) => {
          this.$store.commit("localStorage/setAuthuser", String(resp.user.uid_firebase));
